...
### Hidden (SCEWIN)
*Credit to robbfps*
#### 🛑 **WARNING: SCEWIN IS AN ADVANCED UTILITY THAT IS CAPABLE OF PERMANENTLY BRICKING AND/OR DAMAGING HARDWARE. DO NOT USE SCEWIN IF YOU CANNOT TAKE THIS RISK.**
Hidden settings are not accessible via the BIOS menu, and require use of a utility such as SCEWIN to overwrite these values.
Hidden settings typically relate to power-saving features and disabling more unused features.

### How to use SCEWIN:
Before starting, make sure UAC is disabled in SapphireTool.
1. Navigate to `C:\PostInstall\Tweaks\SCEWIN\5.05.01.0002`
3. Run `export.bat`
- If this fails, please make sure UAC is disabled, and any BIOS write protection is disabled (disablee UEFI variable protection).
5. Copy the exported file `nvram.txt` and rename it to `nvram.txt.bak`
- This backs up your current BIOS settings.
7. Edit `nvram.txt`
- Search for each setting name. Some settings may have multiple copies, so please adjust them as well.
- To edit values, delete and move the asterisk to your selected value.
- It is recommended to backup your modified `nvram.txt` as well.
9. After editing and saving, run `import.bat`
10. Restart your PC
11. Run `import.bat` again and check if values were not modified, meaning it successfully applied. You can also run `export.bat` and check with your backup.

⚠️ **SCEWIN will output warnings/errors that are safe to ignore. For example:**
```
WARNING: Length of string for control ( `Alpha` GUID:) not updated as the value/defaults specified in the script file doesn't reach the minimum range (36).
WARNING: Length of string for control ( `Beta` GUID:) not updated as the value/defaults specified in the script file doesn't reach the minimum range (36).
WARNING: Length of string for control ( `Gamma` GUID:) not updated as the value/defaults specified in the script file doesn't reach the minimum range (36).
Warning : Unmatched question... prompt: GPIO control,Token:5d3
...
...
### Settings:
*Some settings may not exist for your motherboard.*
*Applies to AMD and Intel*

**Interrupt Redirection Mode Selection |** Set to `Round Robin` (Make sure its not `PAIR with Round robin`)
This can help with smoothness and input.

**AP Threads Idle Manner |** Set to `RUN Loop`
This can help with smoothness.

**SelfRefresh Enable |** Set to `Disable`
This can help with input. May cause issues when booting/restarting - notably on low-end motherboards.

**C6DRAM |** Set to `Disable`
This is \elated to C-states. Allows moving DRAM contents to PRM memory when CPU is in C6 state.

**EIST |** Set to `Disable`
Disables Enhanced Intel Speedstep throttling.

**C6 Mode |** Set to `Disable`
Stops the CPU from going into C6 state.

**Global C-state Control |** Set to `Disable`
Related to power savings.

**Opcache Control |** Set to `Disable`
Related to power savings.

**Isochronous Support |** Set to `Disable`
Related to syncing and prioritizing video/audio, which can result in lag.

**AB Clock Gating |** Set to `Disable`
Related to PCI power saving.

**PCIB Clock Run |** Set to `Disable`
Related to PCI power saving.

**Chipset Power Saving Features |** Set to `Disable`
Related to power savings.

**Power Loading |** Set to `Disable`
Useless feature that does not need to be enabled.

**PSPP Policy |** Set to `Performance`
